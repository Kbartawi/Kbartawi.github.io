...
- **Frame:** Voice workspace in internal beta. Build 3.0.0 (4) is verified in Owner Testing (Internal, one tester). Build 3.0.0 (5), adding optional local reminders, uploaded at 06:04:09 Dubai on 20 September; its Apple processing and assignment remain unverified. Build 5 passed 96 automated checks with one device-only skip, plus Simulator reminder permission, schedule/remove, notification delivery and private-note navigation checks. Physical-device acceptance and language quality remain unverified. Optional on-device AI requires iOS 26.4+ and compatible enabled hardware; no third-party AI or automatic sending is connected. Earlier video evidence remains historical.
