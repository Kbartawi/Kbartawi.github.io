...
- **Frame:** the public studio is a fixture-based sample. Native V2 supports resumable drafts, gallery recovery and safer import, export and voice saving; 44 Simulator tests pass. An unsigned iPhone Release archive, version 2.0.0 (1), now builds with valid icon and privacy metadata. Signing and upload await owner Apple Account sign-in in Xcode. The native editor works offline, with no Frame login or AI provider integration. The earlier import/title/resume/export/gallery flow was checked through the UI; the latest controls and physical-device acceptance remain open. The separate web studio offers Kling and Seedance with 174 mocked tests. AI creative acceptance and TestFlight release remain pending.
