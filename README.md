...
- **Frame:** Voice workspace in internal beta. Build 3.0.0 (4) remains verified in Owner Testing (Internal, one tester). Signed build 3.0.0 (6) uploaded at 11:35:59 Dubai on 20 September; its Apple processing and assignment remain unverified. It includes local reminders, foreground-recording guidance, clearer save retries and access to every saved action. The latest candidate passed 25 targeted checks with one device-only skip and 27 iOS 18.5 compatibility checks; Simulator UI verified finding and completing a fourth action. The earlier reminder suite passed 96 checks with one device-only skip. Physical-device acceptance and language quality remain unverified. Optional local AI requires iOS 26.4+ and compatible enabled hardware; no third-party AI or automatic sending is connected.
