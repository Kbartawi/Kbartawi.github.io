...
- **Frame:** public studio is a fixture-based sample. The native iPhone editor now saves resumable drafts, passes 12 Simulator tests and completed photo import → typed title → leave/resume → local export → gallery through the UI. The web studio offers lower-cost Kling 3.0 Standard and Seedance 2.5 with explicit estimates and protected existing jobs; 174 mocked backend tests pass. The earlier local AI film was creatively rejected. OpenAI voice acceptance, paid Kling quality comparison, physical-device checks and TestFlight remain pending.
